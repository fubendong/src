...
class GoodsSearchSlider extends Base
{
    ...
    public function get($f3)
    {
        ...
        $smarty->assign(
            'goods_search_adv_slider',
            json_decode(ShopThemePlugin::getOptionValue('goods_search_adv_slider'), true)
        );

        $smarty->display('theme_shop_goods_search_slider.tpl', 'get');
    }
    ...
    public function post($f3)
    {
        ...
        ShopThemePlugin::saveOptionValue(
            'goods_search_adv_slider',
            json_encode($goods_search_adv_slider)
        );

        // 清除搜索页面的缓存
        $shopClear = new ShopClear();
        $shopClear->clearGoodsSearch();

        $this->addFlashMessage('保存设置成功');

        RouteHelper::reRoute($this, '/Theme/Shop/GoodsSearchSlider');
    }
}
